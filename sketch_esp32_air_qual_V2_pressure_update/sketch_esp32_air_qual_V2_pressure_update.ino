...
void loop() {
    ...
    if (readError) {
        ...
        if (bmp388Ready && bmp388.performReading()) {
            bmpRawPressure   = bmp388.pressure / 100.0f; // raw absolute station pressure in hPa
            bmpTempC         = bmp388.temperature;
            
            // Manual calculation for Sea Level Pressure (Normalized for weather tracking)
            float kelvin = bmpTempC + 273.15f;
            seaLevelPressure = bmpRawPressure * powf(1.0f - (0.0065f * MY_ELEVATION_METERS) / (kelvin + (0.0065f * MY_ELEVATION_METERS)), -5.257f);
        }
